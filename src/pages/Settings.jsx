...
async function adminFetch(path, options = {}) {
  ...
  let res
  try {
    res = await callFunction(`admin-users${path}`, {
      ...options,
      token: accessToken,
      // Reading the user list is safe to repeat; creating or deleting one is not.
      attempts: options.method && options.method !== 'GET' ? 1 : 2,
    })
  } catch (err) {
    return { error: getErrorMessage(err, 'Could not reach the user service.') }
  }
  if (res.status === 401) {
    await supabase.auth.signOut().catch(() => {})
    return { error: 'Your login session expired. Sign in again before managing users.' }
...
export default function Settings() {
  ...
  async function sendTestNotification() {
    ...
    await callFunction('send-push', {
      ...
      body: { title: 'Test', body: 'Push notifications are working!', url: null, to_self: true },
    }).catch(() => {})
