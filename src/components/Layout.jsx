...
import { useStorageViewTracker } from '@/lib/storageViewTracker'
...
function ReservationBanner() {
  ...
  return (
    <div className="flex flex-col gap-1 px-3 pt-2 flex-shrink-0 print-hide">
      {visible.map(r => (
...
export default function Layout() {
  ...
  return (
    <div className="flex flex-col h-full print-flow">
      {showLogo && (
        <div className="flex items-center justify-between gap-3 px-4 pt-3 pb-1 border-b bg-background flex-shrink-0 print-hide">
          <img src="/logo.webp" alt="Timberfell" className="h-8 w-auto logo-invert" />
          <ConnectionStatus />
        </div>
      )}
      {isVoiceDeploy && (
        <div className="flex items-center justify-between px-2 py-1 flex-shrink-0 print-hide">
          <button
            type="button"
            onClick={() => navigate(-1)}
      ...
      <main className="flex-1 overflow-hidden print-flow">
        <Outlet />
      </main>
      {showNav && <BottomNav />}
      <PushPrompt />
